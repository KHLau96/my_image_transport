...
int main(int argc, char** argv)
{
    ...
    BOOL bMonoCamera = tCapability.sIspCapacity.bMonoSensor;
    if (bMonoCamera)
    {
    	CameraSetIspOutFormat(hCamera, CAMERA_MEDIA_TYPE_MONO8);
    }

    //SetCameraResolution(hCamera,240,212,1040,812);

    //软性设置帧率
    CameraSetTriggerMode(hCamera,1);
    CameraSetTriggerCount(hCamera,0xFFFFFFFF);
    CameraSetExtTrigIntervalTime(hCamera, 50*1000);
    CameraSoftTrigger(hCamera);

    //设置曝光时间
    CameraSetAeState(hCamera,FALSE);
    CameraSetExposureTime(hCamera,20 * 1000);
    //SetCameraResolution(hCamera,0,0,640,480);
    ...
    */
    ...
    while(ros::ok())
    {
        if(CameraGetImageBuffer(hCamera,&sFrameInfo,&pbyBuffer,1000) == CAMERA_STATUS_SUCCESS)
		{
			...
			resize(matImage,matImage,Size(800,600));
			
			cv_bridge::CvImage brige;
			brige.header.stamp = ros::Time::now();
			brige.header.frame_id = "camera_link";
			brige.image = matImage;
			brige.encoding = "mono8";


			sensor_msgs::ImagePtr msg = brige.toImageMsg();
			//msg.Header
            pub.publish(msg);
